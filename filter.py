...
def in_list(hi: list[Customer], filterr: str) -> bool:
    """ Return a description of this filter to be displayed in the UI menu
"""
    for char in hi:
        if filterr.isdigit() and char.get_id() == int(filterr):
            return True
    return False
...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        """
        if not in_list(customers, filter_string):
            return data
        hi = []
        second = []
        for c in customers:
            if c.get_id() == int(filter_string):
                hi = hi + (c.get_history()[0])
                hi = hi + (c.get_history()[1])
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        """
        try:
            if filter_string[0] not in "LG" or not \
                    filter_string[1::].isdigit() or \
                    len(filter_string) > 5:
                return data
            hi = []
            if filter_string[0] == "L":
                for c in data:
                    if c.duration < float(filter_string[1::]):
                        hi.append(c)
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        try:
            hi = []
            tool = (filter_string.split(", "))
            lowerLong = getInt(tool[0])
            lowerlat = getInt(tool[1])
            upperlong = getInt(tool[2])
            upperlat = getInt(tool[3])
            "(-79.45335277309937, 43.61125095762723, -79.5926073826452, 43.6109938420157)"
            for char in data:
                if (lowerLong <= char.src_loc[0] and lowerlat
                                                       <= char.src_loc[1]) and (
                        upperlong >= char.src_loc[0]) and \
                        upperlat >= getInt(str(char.src_loc[1])):
                    hi.append(char)
                if (lowerLong <= char.dst_loc[0] and lowerlat
                                                             <= char.dst_loc[
                                                                 1]) and \
                        (upperlong >= char.dst_loc[0]) and \
                        upperlat >= getInt(str(char.dst_loc[1])):
                    hi.append(char)
            return list(set(hi))
        except:
            return data
